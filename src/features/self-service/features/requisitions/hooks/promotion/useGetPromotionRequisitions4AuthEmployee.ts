...
export const QUERY_KEY_FOR_PROMOTION_REQUISITIONS_FOR_AUTH_EMPLOYEE =
  "promotion-requisitions-for-auth-employee";
...
const getData = async (props: {
  ...
  const url = `${MICROSERVICE_ENDPOINTS.UTILITY}/self-service/requisition/promotion/mine`;
  ...
  const data: TPromotionRequisition[] = result.map(
    (item: TPromotionRequisition): TPromotionRequisition => ({
      ...item,
    })
  );
...
export const useGetPromotionRequisitions4AuthEmployee = (
  props: IGetDataProps
) => {
