...
const LeaveBankOverviewDetails: React.FC<
...
> = ({ open, handleClose, data }) => {
  return (
    <Modal
      ...
      style={{ top: 120 }}
    >
      <p className="mt-2">The breakdown reflects your current leave bank.</p>
      <table className="w-full text-center mt-6">
        ...
        <tbody>
          {data?.map(({ name, length, used }, i) => (
            <tr key={i}>
              <td className="p-1 capitalize text-left">{name}</td>
              <td>{formatNumberWithCommas(length, 0)}</td>
              <td>{formatNumberWithCommas(used, 0)}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
export default LeaveBankOverviewDetails;
