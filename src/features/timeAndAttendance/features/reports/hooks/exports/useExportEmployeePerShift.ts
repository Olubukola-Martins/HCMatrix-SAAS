...
import { getEmployeePerShift } from "../useGetEmployeePerShift";
...
type TProps = Omit<Parameters<typeof getEmployeePerShift>[0], "auth">;
...
const generateExport = async ({
  ...
  try {
    const data = await getEmployeePerShift({ auth, ...props });
    if (data.total === 0) {
      return {
    ...
    exportEntityAsFile({
      fileName,
      entityRows: dataRows,
      workSheetName: "Per_shift_Report",
    });
...
export const useExportEmployeePerShift = ({
  handleSuccess,
}: { handleSuccess?: () => void } = {}) => {
