...
export const QUERY_KEY_FOR_PAYSLIP_TEMPLATES = "payslip-templates";
...
const getData = async (props: {
  ...
  const url = `${MICROSERVICE_ENDPOINTS.PAYROLL}/payslip/template`;
...
export const useGetPayslipTemplates = (props: IGetDataProps) => {
  const { token, companyId } = useApiAuth();

  const { pagination, searchParams } = props;
  const queryData = useQuery(
    [QUERY_KEY_FOR_PAYSLIP_TEMPLATES, pagination, searchParams],
    () =>
      getData({
